...
class LinearAnnealedPolicy(Policy):
	...
	self.testlib = ctypes.CDLL('/home/fei/Documents/Isaac_DRL/penaltyDRL/goalkeeperDRL/examples/blackboard/blackboard.so') #chama a lybrary que contem as funcoes em c
...
#   metodo com decremento exponencial
    def get_current_value(self):
        pointer = self.testlib.leitura_int()
        if self.agent.training:
            # exponencial annealed: f(x) = 1/exp(2ex).
            a = 1.0 - self.value_min
            b = np.exp(float((2.0*float(self.agent.step)*np.exp(1))/float(self.nb_steps)))
            y = a/b + self.value_min
            value = max(self.value_min, y)
        else:
            value = self.value_test
        self.testlib.escreve_int(1000013, int(value*1000000))
        return value

#   metodo com decremento linear
#    def get_current_value(self):
#	    pointer = self.testlib.leitura_int()
#        if self.agent.training:
